...
def count_language_indicators(text: str) -> Tuple[int, int]:
    ...
    spanish_count = sum(len(re.findall(pattern, text, re.IGNORECASE)) for pattern in SPANISH_INDICATORS)
    english_count = sum(len(re.findall(pattern, text, re.IGNORECASE)) for pattern in ENGLISH_INDICATORS)
...
def generate_report(results: Dict[str, List[Tuple[str, str, str]]]) -> str:
    ...
    for module_name in sorted(results.keys()):
        ...
        report.append(f"**Status**: {eng_count} English | {spa_count} Spanish | {mix_count} Mixed | {unk_count} Unknown\n")
        report.append(f"\n| File | Type | Language |\n")
        report.append(f"|------|------|----------|\n")
    ...
    conversion_needed = []
    for module_name in sorted(results.keys()):
        for file_path, file_type, language in sorted(results[module_name]):
